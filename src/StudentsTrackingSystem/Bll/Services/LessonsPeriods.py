...
class LessonsPeriodsService:
    ...
    def create_period(self, data: LessonPeriodCreate) -> LessonPeriodDetail:
        ...
        if data.start_time >= data.end_time:
            raise BusinessValidationError("Ошибка: Время начала не может быть раньше времени окончания!")
        if self.period_repo.get_by_start_time(data.start_time) is not None:
            raise ConflictError(
                f"Период с началом в {data.start_time.strftime('%H:%M')} уже существует"
            )
    ...
    def get_by_id(self, period_id: int) -> LessonPeriodDetail:
        ...
        if period is None:
            raise NotFoundError("Ошибка: не найден!")
    ...
    def update_period(self, period_id: int, data: LessonPeriodUpdate) -> LessonPeriodDetail:
        ...
        if new_start_time >= new_end_time:
            raise BusinessValidationError("Ошибка: Время начала не может быть раньше времени окончания!")
        if new_start_time != period.start_time:
            if self.period_repo.get_by_start_time(new_start_time) is not None:
                raise ConflictError(
                f"Период с началом в {data.start_time.strftime('%H:%M')} уже существует"
            )
